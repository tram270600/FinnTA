...
export const createRoom = createAsyncThunk<room, string, { rejectValue: contactError }>(
    '/chat/createRoom',
    async (id, thunkApi) => {
        let res = await talker.Conn.post<room>('/chat/createRoom', JSON.stringify({ "ID": id }))
        if (res.status !== 200)
...
export const getRoom = createAsyncThunk<room[], number, { rejectValue: contactError }>(
    '/chat/room',
    async (page, thunkApi) => {
        let res = await talker.Conn.get<room[]>('/chat/room', { params: { page: page } })
        if (res.status !== 200)
            return thunkApi.rejectWithValue({
...
export const getChat = createAsyncThunk<chat[], { room_id: string, page: number }, { rejectValue: contactError }>(
    '/chat/get',
    async (data, thunkApi) => {
        let res = await talker.Conn.get<chat[]>('/chat', { params: { page: data.page, room: data.room_id } })
        if (res.status !== 200)
            return thunkApi.rejectWithValue({
